    ...
    exportSets (): Set[] {
        return this.sets.map((set) => {
            ...
            return ({
                ...
                entrant1ID: leftEntrant ? leftEntrant.entrantID : undefined,
                entrant2ID: rightEntrant ? rightEntrant.entrantID : undefined,
                entrant1Result: entrant1Result as SetGameResult,
                entrant2Result: entrant2Result as SetGameResult,
                entrant1Score,
                entrant2Score,
                entrant1NextSetID: parentSet ? `${parentSet.setId}` : undefined,
                entrant2NextSetID: parentSet ? `${parentSet.setId}` : undefined,
                entrant1PrevSetID: leftSet ? `${leftSet.setId}` : leftUpperBracketSet ? `${leftUpperBracketSet.setId}` : undefined,
                entrant2PrevSetID: rightSet ? `${rightSet.setId}` : rightUpperBracketSet ? `${rightUpperBracketSet.setId}` : undefined,
                games: [],
                other: {
                    matchLimit,
                    nextWinnerSet: parentSet ? `${parentSet.setId}` : undefined,
                    nextLoserSet: loserSet && `${loserSet.setId}`,
                    nextLeftWinnerSlot: parentSet ? `${parentSet.setId}` : undefined,
                    nextRightWinnerSlot: parentSet ? `${parentSet.setId}` : undefined,
                    nextLeftLoserSlot: loserSet && loserSet.leftUpperBracketSet && loserSet.leftUpperBracketSet.setId === setId ? loserSet.setId : undefined,
                    nextRightLoserSlot: loserSet && loserSet.rightUpperBracketSet && loserSet.rightUpperBracketSet.setId === setId ? loserSet.setId : undefined,
